...
public class AddMultiplePreparationStepsScenario {
    ...
    @And("adds another preparation step {string} with ingredient {string} to the recipe {string}")
    public void addAnotherPreparationStepWithIngredient(String stepDescription, String ingredientName, String recipeName) {
    ...
    @And("recipe {string} has another preparation step {short} as {string}")
    public void recipeHasAnotherPreparationStepAs(String recipeName, Short stepNum, String stepDescription) {
        List<Short> recipeIds = recipeService.getRecipeIdsByName(recipeName);
        Short recipeId = recipeIds.get(0);
        preparationStepInDatabase = preparationStepService.getPreparationStepsByRecipeId(recipeId).get(1);
    ...
    @And("the second preparation step has ingredient {string}")
